...
#undef _WRS_CONFIG_SMP /* define:双核模式 undef:单核模式 */
#endif
...
#define VXB_NS16550_SPIN_LOCK_ISR_TAKE(pChan)   if (ns16550SpinlockFuncReady)SPIN_LOCK_ISR_TAKE(&pChan->spinlockIsr)
#define VXB_NS16550_SPIN_LOCK_ISR_GIVE(pChan)   if (ns16550SpinlockFuncReady)SPIN_LOCK_ISR_GIVE(&pChan->spinlockIsr)
#define VXB_NS16550_SPIN_LOCK_ISR_HELD(pChan)   (ns16550SpinlockFuncReady ? spinLockIsrHeld(&pChan->spinlockIsr) : FALSE)
#define VXB_NS16550_ISR_SET(pChan)              SPIN_LOCK_ISR_TAKE(&pChan->spinlockIsr)
#define VXB_NS16550_ISR_CLEAR(pChan)            SPIN_LOCK_ISR_GIVE(&pChan->spinlockIsr)
#define VXB_NS16550_SPIN_LOCK_READY             ns16550SpinlockFuncReady = TRUE
#else
#define VXB_NS16550_SPIN_LOCK_ISR_TAKE(pChan)   SPIN_LOCK_ISR_TAKE(&pChan->spinlockIsr)
#define VXB_NS16550_SPIN_LOCK_ISR_GIVE(pChan)   SPIN_LOCK_ISR_GIVE(&pChan->spinlockIsr)
...
#define NS16550_MAX_RATE            1500000
...
#define REG_GET(reg, pchan, val)    (val = vxbRead8 (pchan->handle, (UINT8 *)pchan->pDev->pRegBase[pchan->regIndex] + (reg * pchan->regDelta)));
#define REG_SET(reg, pchan, val)    (vxbWrite8 (pchan->handle, (UINT8 *)pchan->pDev->pRegBase[pchan->regIndex] + (reg * pchan->regDelta), val));
...
)
{
    ...
    else if (pDev->busID == VXB_BUSID_PLB)
    {
        ...
        if (pHcf != NULL)
        {
             ...
             * 8-bit registers. }
             */
            devResourceGet (pHcf, "regInterval", HCF_RES_INT, (void *)&regInterval);
        }
    }
    ...
    /* IER[ETXRDY] <- 0 */
    /* IER[ETXRDY] <- 1 */
    regVal = vxbRead8 (handle, pDev->pRegBase[regBaseIndex] + (IER * regInterval));

    regVal &= ~IER_ETHREI;

    vxbWrite8 (handle, pDev->pRegBase[regBaseIndex] + (IER * regInterval), regVal);
    ...
    vxbWrite8 (handle, pDev->pRegBase[regBaseIndex] + (THR * regInterval), regVal);
    ...
    vxbWrite8 (handle, pDev->pRegBase[regBaseIndex] + (LCR * regInterval), regVal);

    /*
     * Check for pending interrupts by reading the Interrupt ID Register
     */
    regVal = vxbRead8 (handle, pDev->pRegBase[regBaseIndex] + (IIR * regInterval));
    ...
    /* reset loopback */
    /* MCR[LOOPBACK] <- 0 */
    regVal = vxbRead8 (handle, pDev->pRegBase[regBaseIndex] + (MCR * regInterval));

    regVal &= ~MCR_LOOP;

    vxbWrite8 (handle, pDev->pRegBase[regBaseIndex] + (MCR * regInterval), regVal);
...
)
{
    ...
    if ( pDev->busID == VXB_BUSID_PCI )
    {
        ...
        if ( pPciDevInfo->pciVendId == UART_VENDOR_EASYSYNC )
        {
            ...
            /* we don't currently support the ESD1004 */
            if ( ( pPciDevInfo->pciDevId == UART_EASYSYNC_ESD1004 ) || ( pPciDevInfo->pciDevId == UART_EASYSYNC_ESD1004_1 ) )
            {
        ...
        if ( ( pPciDevInfo->pciVendId == UART_VENDOR_OXFORDSEMI ) || ( pPciDevInfo->pciVendId == UART_VENDOR_AXXON ) )
        {
            ...
            else if ( ( pPciDevInfo->pciDevId == UART_DEVID_AXXON_LF653KB_FUNC0 ) || ( pPciDevInfo->pciDevId == UART_DEVID_AXXON_LF662KB_FUNC0 ) )
            {
                /*
            ...
            else if ( ( pPciDevInfo->pciDevId == UART_DEVID_AXXON_LF769KB_1S1P_FUNC1 ) || ( pPciDevInfo->pciDevId == UART_DEVID_AXXON_LF769KB_2S_FUNC0 ) || ( pPciDevInfo->pciDevId == UART_DEVID_AXXON_LF769KB_2S_FUNC1 ) )
            {
                /*
    ...
    for ( i = 0 ; i < 10 ; i++ )
    {
        ...
        if (pDev->busID == VXB_BUSID_PLB)
        {
             ...
             * oscillator. }
             */
            if (devResourceGet(pHcf, "clkFreq", HCF_RES_INT, (void *)&pChan->xtal) != OK)
            {
                /*
                 ...
                 * frequency. }
                 */
                if (devResourceGet(pHcf, "clkFreq", HCF_RES_ADDR, (void *)&pFunc) != OK)
                {
                    hwMemFree ((char *)pChan);
                    return;
                }
             ...
             * calculations. }
             */
            if (devResourceGet(pHcf, "divisorCalc", HCF_RES_ADDR, (void *)&pFunc) != OK)
             ...
             * 8-bit registers. }
             */
            if (devResourceGet (pHcf, "regInterval", HCF_RES_INT, (void *)&val) == OK)
             ...
             */
            if (devResourceGet (pHcf, "fifoLen", HCF_RES_INT, (void *)&val) == OK)
             ...
             * register. }
             */
            if (devResourceGet (pHcf, "ierInit", HCF_RES_INT, (void *)&val) == OK)
                pChan->ierInit = (UINT8) 0; /* BSP中没有这个变量 强行赋值 */
        }
        else
        {
            if ( pDev->busID == VXB_BUSID_PCI )
         ...
         */
        if ( pHcf != NULL && devResourceGet (pHcf, "handle", HCF_RES_ADDR, (void *)&val) == OK)
...
)
{
     ...
     */
    sioEnableFunc = vxbDevMethodGet (vxbDevParent (pDev), (UINT32)&sioEnable_desc);
...
)
{
    ...
    sioIntEnableFunc = vxbDevMethodGet (vxbDevParent (pDev), (UINT32)&sioIntEnable_desc);
...
)
{
    ...
    {
    case CS5:
        pChan->lcr = CHAR_LEN_5;
        break;
    case CS6:
        pChan->lcr = CHAR_LEN_6;
        break;
    case CS7:
        pChan->lcr = CHAR_LEN_7;
        break;
    case CS8:
    default:
        pChan->lcr = CHAR_LEN_8;
        break;
    }
    ...
    case PARENB|PARODD:
        ...
        break;
    case PARENB:
        pChan->lcr |= (LCR_PEN | LCR_EPS);
        break;
    default:
    case 0:
        pChan->lcr |= PARITY_NONE;
        break;
    }
    ...
    if (pChan->fifoSize == NS16550_FIFO_DEPTH_UNKNOWN)
    {
        ...
        if ((value & (FCR_RXTRIG_L | FCR_RXTRIG_H)) == (FCR_RXTRIG_L | FCR_RXTRIG_H))
        {
            /*
             * FIFO is supported.
    ...
    value = (FCR_DMA | RxCLEAR | TxCLEAR | FIFO_ENABLE | FCR_RXTRIG_H);
    REG_SET(FCR, pChan, value);
...
)
{
    ...
    if (baud > 115200)
    {
        switch((int)pChan->pDev->pRegBase[0]) /* 根据基地址获取串口号 当地址不标准时需要注意 */
        {
        case 0x3f8:
            uchCom = 0x04;
            break;
        ...
        */
        ...
        sysOutByte(0x2e, 0x55);     /* Entering the Configuration State */
        sysOutByte(0x2e, 0x07);     /* Entering Logical Device Configuration */
        sysOutByte(0x2f, uchCom);   /* Select SERIAL PORT */
        sysOutByte(0x2e, 0xf0);     /* Serial Port Mode Register*/
        sysOutByte(0x2f, uchReg);
        sysOutByte(0x2e, 0xaa);     /* Exit the Configuration State */
    }
...
)
{
    ...
    while (TRUE)
    {
        ...
        if ((status & LSR_THRE) != 0x00)
        {
            ...
            for (i = 0; i < pChan->fifoSize; i++)
            {
                if ((*pChan->getTxChar) (pChan->getTxArg, &outChar) != ERROR)
                {
                    ...
                    /* 增加485支持 */
                    REG_SET(MCR, pChan, MCR_RTS | MCR_OUT2);
        ...
        else
        {
            ...
            for (;;)
            {
            	REG_GET(LSR, pChan, uch);
            	if ((uch & 0x60) == 0x60)
				{
					break;
				}
            }
            REG_SET(MCR, pChan, MCR_OUT2);
...
)
{
    ...
    for ( i = tyCoMin ; i <= tyCoMax ; i++ )
    {
        ...
        for ( baud = baudMin ; baud <= baudMax ; baud *= 2 )
        {
            ...
            ctr = 0;
            do
                retVal = ns16550vxbPollOutput((NS16550VXB_CHAN *)sysSerialChanGet(i), 'a' + i);
